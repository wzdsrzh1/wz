...
class Trainer:
    ...
    def __init__(self, config):
        ...
        print(f"\n{'='*60}")
        print(f"模型总参数量: {total_params:,} ({total_params * 4 / 1e6:.2f}MB)")
    ...
    def train_epoch(self, train_loader):
        ...
        for batch_idx, (img1, img2) in enumerate(pbar):
            ...
            if (batch_idx + 1) % self.config.log_interval == 0:
                ...
                loss_str = ', '.join([f'{k}: {v/(batch_idx+1):.4f}' 
                                     for k, v in total_loss_dict.items() if k != 'total'])
                pbar.set_postfix({
                    'loss': f'{avg_loss:.4f}',
                    'details': loss_str
                })
        ...
        return epoch_loss, total_loss_dict
    ...
    def train(self, train_loader, val_loader=None):
        ...
        for epoch in range(self.current_epoch, self.config.epochs):
            ...
            # 保存损失
            self.save_losses(train_loss_dict, val_loss_dict)
...
def main():
    ...
    print("加载验证数据...")
    val_loader = create_dataloaders_test(
        image_size=config.image_size,
        batch_size=config.batch_size,
        num_workers=config.num_workers
    )
